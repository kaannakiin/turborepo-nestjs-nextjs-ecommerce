-- Bu kodu yeni oluşturulacak migration.sql dosyasına yapıştır
...
WITH base_products AS (
  SELECT
    ...
    p."visibleAllCombinations", -- <-- YENİ ALAN EKLENDİ
    p."brandId",
    ...
    (SELECT jsonb_agg(jsonb_build_object('discountId', dc."discountId")) FROM "DiscountIncludedProduct" dip JOIN "DiscountCondition" dc ON dip."conditionId" = dc.id WHERE dip."productId" = p.id) AS "discountIncluded",
...
variant_products AS (
  SELECT
    ...
    (SELECT jsonb_agg(jsonb_build_object('id', pa.id, 'order', pa.order, 'url', a.url, 'type', a.type) ORDER BY pa.order ASC) FROM "ProductAsset" AS pa JOIN "Asset" AS a ON pa."assetId" = a.id WHERE pa."productId" = p.id) AS "productAssets",
    (SELECT jsonb_agg(jsonb_build_object('id', c.id, 'name', ct.name, 'slug', ct.slug)) FROM "ProductCategory" pc JOIN "Category" c ON pc."categoryId" = c.id LEFT JOIN "CategoryTranslation" ct ON c.id = ct."categoryId" AND ct.locale = 'TR' WHERE pc."productId" = p.id) as categories,
    (SELECT jsonb_agg(jsonb_build_object('discountId', dc."discountId")) FROM "DiscountIncludedVariant" div JOIN "DiscountCondition" dc ON div."conditionId" = dc.id WHERE div."combinationId" = pvc.id) AS "discountIncluded",
