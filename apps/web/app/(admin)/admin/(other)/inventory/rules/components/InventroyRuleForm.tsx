...
import { useUpsertInventoryRule } from '@hooks/admin/useInventory';
...
import {
  ...
  FullfillmentDecisionTreeSchema,
...
interface InventoryRuleFormProps {
  defaultValues?: FullfillmentStrategyInput;
}
...
const getInventoryNodeLabel = (node: Node) => {
  ...
  if (node.type === 'condition') {
    const data = node.data as { condition: FullfillmentCondition };
...
const InventoryRuleForm = ({ defaultValues }: InventoryRuleFormProps) => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FullfillmentStrategyInput>({
    resolver: zodResolver(FullfillmentStrategyZodSchema),
    defaultValues: defaultValues || {
      name: '',
      description: '',
      isActive: true,
      isDefault: false,
      type: FullfillmentStrategyType.PROXIMITY,
      priority: 0,
      settings: {
  ...
  const upsertMutation = useUpsertInventoryRule();
  ...
  const handleFlowSave = (data: { nodes: Node[]; edges: Edge[] }) => {
    try {
      const validated = FullfillmentDecisionTreeSchema.parse({
        nodes: data.nodes,
        edges: data.edges,
      });
  ...
  const onSubmit: SubmitHandler<FullfillmentStrategyOutput> = (data) => {
    upsertMutation.mutate(data, {
      onSuccess: () => {
        notifications.show({
          message: `${data.name || 'Fulfillment Stratejisi'} başarıyla kaydedildi.`,
          color: 'green',
        });
        router.push('/admin/inventory/rules');
      },
      onError: (error) => {
        notifications.show({
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Stack gap="lg">
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Select
                      ...
                      data={Object.keys(FullfillmentStrategyType).map(
                        (key) => ({
                          value: key,
                          label: getFulfillmentStrategyTypeLabel(
                            key as FullfillmentStrategyType,
                          ),
                        }),
                      )}
                      allowDeselect={false}
          ...
          <Group justify="flex-end" mt="xl">
            <Button
              variant="default"
              onClick={() => router.push('/admin/inventory/rules')}
            >
              İptal
            </Button>
            <Button type="submit">
      ...
      <FlowDrawer<FullfillmentConditionField, FullfillmentCondition>
        opened={drawerOpened}
        onClose={closeDrawer}
